...
#include <stdio.h>
#include <stdlib.h>
#include "../include/NMPT/NMPTUtils.h" 
#include "../include/NMPT/BlockTimer.h"
#include "../include/NMPT/DebugGlobals.h"
...
void FastSalience::updateSalience(const Mat &frame)  {
	...
	if (colorframe.channels() != 1) {
		if (!useColor) {
			cvtColor(colorframe, redChannel, colorframe.channels()==3 ? CV_BGR2GRAY : CV_BGRA2GRAY); 
		} else {
...
vector<KeyPoint> FastSalience::getKeyPoints(int radius) const{
	...
	for (int i = borderr; i < h-borderr; i++) {
		for (int j = borderc; j < w-borderc; j++) {
			if (kept.at<unsigned char>( i, j) > 0) {
				KeyPoint pt; 
				pt.pt = Point(j,i); 
				pts.push_back(pt); 
			}
